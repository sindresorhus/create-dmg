...
async function composeIcon(type, appIcon, mountIcon, composedIcon) {
	...
	const mountIconSize = await promisify(appIcon.size).bind(appIcon)();
	...
	await promisify(appIcon.write).call(appIcon, tempAppIconPath);
	// Compose the two icons
	mountIcon = mountIcon.composite(tempAppIconPath).gravity('Center').geometry('+0-' + (mountIconSize.height * 0.155));

	composedIcon[type] = await promisify(mountIcon.toBuffer).call(mountIcon);
}

const hasGm = async () => {
	try {
		const result = await execa('gm', ['-version']);
